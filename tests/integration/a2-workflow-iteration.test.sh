...
if scenario == "green":
    ...
    ledger.set_winner_step_id(repo, run_id, "judge", "plan-1")
...
it "G1 / ADV-R2-3 DF: init_ledger handles Unicode-superscript step ids without crashing (isdecimal vs isdigit)"
res="$("$PY" - "$AUTO_ROOT" <<'PYEOF'
...
case "$res" in
  *OK*) pass ;;
  *) fail "G1 DF: init_ledger raised (or wrong seed) on Unicode-superscript step id: $res" ;;
esac

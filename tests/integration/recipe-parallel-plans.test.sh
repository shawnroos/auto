...
#
# Scenarios:
#   1. green: judge calls set_winner_step_id("plan-2") → producer emits plan-2's
...
# Returns CSV:
#   loop_phase | work_step_ids_in_phase_work (sorted, comma-joined)
# Fix-pass I (round-2 P0): the judge writes its verdict via the PRODUCTION
# write path — record_verdict for findings, set_winner_step_id for the
...
# ─── Scenario 1: green — producer emits the winner's enumerated_steps ────────
it "fix-pass C T2 GREEN: a2 with judge winner=plan-2 → producer emits plan-2's steps"
res="$(drive_a2 green)"
# After auto-flip the producer ran inside transition_and_emit; loop_phase=work,
# new work steps = plan-2's enumerated set (wB-1, wB-2). The judge step is
...
# ─── Scenario 2: deliberate-fail — judge findings have no winner_step_id ────
...
# ─── Scenario 3: malformed judge — winner_step_id names a non-existent step ─
...
# the "no winner_step_id" raise now names dispatch_context + set_winner_step_id
# so the operator knows EXACTLY where to look and what to call.
it "fix-pass I T2 MALFORMED MESSAGE: producer raise names dispatch_context + set_winner_step_id"
...
# winner_step_id from findings → production A2 was unrunnable. Fix-pass I
# moved the field onto dispatch_context.winner_step_id, written by a new
# mutator set_winner_step_id. This test proves the full production path now
# works: dispatch judge, record_verdict (still strips findings — that's
# correct/intended), set_winner_step_id, then the producer reads the winner
# from dispatch_context and emits the winning plan's steps.
it "fix-pass I T2 PRODUCTION PATH: record_verdict + set_winner_step_id → producer wires the winner"
...
# winner_step_id were stripped on the way to disk, this would emit [] and the
# assertion would break — proving the production path is now closed.
assert_eq "wB-1,wB-2" "$prod_result"

# Defense-in-depth check: set_winner_step_id rejects an unknown winner BEFORE
...
it "fix-pass I T2 WRITE-BOUNDARY GUARD: set_winner_step_id rejects unknown winner with the bad id in the message"
...
# Round-3 P3 promotion (fix-pass J): set_winner_step_id must REJECT a judge
# naming itself as winner. The previous existence check was over the full
# step set, so judge could be its own winner — guard passed, but the producer
# would silently emit [] (judges don't carry enumerated_steps). The fix
# excludes judge_step_id from the eligible set; the malformed case now
# surfaces at the write boundary with a message naming the constraint.
it "fix-pass J: set_winner_step_id rejects judge naming itself as winner (P3 promotion)"

...
# Build a terminal, defect-free, single-step ledger -> met should be true.
...
for i,(frm,to) in enumerate(edges):
    ...
    try:
        ...
        pass
    # fresh ledger per edge, step seeded in `frm`
    import os
    p = m.ledger_path(repo, run)
    if os.path.exists(p): os.unlink(p)
    m.init_ledger(repo, run, backend="ce", steps=[{"id":"U1","state":frm}])
    try:
...
#   lib/ledger.py step_is_terminal: force  gating = GATING_SEVERITIES  (drop the
...
# work-phase ledger with steps==[] NOT met. A contrasting work run WITH one
# terminal step IS met — proving the guard is the empty-set check, not a blanket
# false on the work phase.
#
# Verify-RED: lib/ledger.py recompute_predicate, drop the `and has_steps` conjunct
...
m = importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
try:
    m.init_ledger(repo, run, backend="native", steps=[{"id":"X"}])
...
m = importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
try:
    m.init_ledger(repo, run, backend="ce", steps=[{"id":"U1","state":"pending"}])
    print("won")
except m.LedgerExists:
    print("exists")
...
m.init_ledger(repo, run, backend="ce", steps=[{"id": "U1"}])
u = m.read_ledger(repo, run)["steps"][0]
...
# phase defaults to plan (a step with no phase in a plan-default ledger),
...
nu = m.ledger_core._normalize_step({"id": "G1", "verification": crit})
...
spec = importlib.util.spec_from_file_location("ledger", sys.argv[1])
m = importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
nu = m.ledger_core._normalize_step({"id": "U1"})
...
it "U3: round-trip init_ledger -> read_ledger retains the gate step's verification"

...
lifted it HERE so a second validator — ``lib/contents.py::validate_content`` —
can check a content's op against the SAME set without importing the orchestrator
...
``orchestrator.py`` and ``contents.py`` import THIS, so there is ONE definition;
``tests/unit/contents.test.sh`` asserts ``adapter_ops.VALID_ADAPTER_OPS ==
...
# The four ops a V1 recipe/content unit may declare. Kept as a frozenset so

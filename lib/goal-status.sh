#!/usr/bin/env bash
# claude-dispatch U7: deterministic loop-status surface for the engine's OWN
...
#   feed it the ledger result. So claude-dispatch ships its OWN thin Stop hook
...
# claude_dispatch::goal_status <repo> <run>
...
claude_dispatch::goal_status() {
  local repo="${1:-}"
  local run="${2:-}"
  local script_dir
  script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

  "$CLAUDE_DISPATCH_PYTHON3" "${script_dir}/goal-status.py" "$repo" "$run"
}

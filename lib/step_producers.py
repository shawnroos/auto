...
them into ledger steps at the phase boundary.

PRODUCERS ARE PURE (F3): each is `(ledger, to_phase) -> list[new_step_dict]`. They
READ the ledger dict and RETURN new partial step dicts. They MUST NOT call ledger
...
def _plan_steps(ledger: dict) -> list:
    return [u for u in ledger.get("steps", []) if u.get("phase") == "plan"]


def _brainstorm_steps(ledger: dict) -> list:
...
def brainstorm_output_to_plan_step(ledger: dict, to_phase: str) -> list:
    ...
    a one-element list of a partial 5-key step dict; no ledger mutation. The
...
def judge_winner_to_work_steps(ledger: dict, to_phase: str) -> list:
    ...
    The winner_id is written by ``ledger.set_winner_step_id(judge_step_id,

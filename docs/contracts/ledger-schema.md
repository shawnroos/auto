...
| `steps` | array | per-unit ledger entries; see §2.3 |
...
| `all_steps_terminal` | bool | `true` iff EVERY unit is terminal (see "terminal" definition, §4, I-2) |
...
`all_steps_terminal == true` iff `terminal(u)` holds for every `u in units`.
...
`all_steps_terminal` is NOT a requirement (it would never hold while units are
...
| `init_ledger(repo_root, run_id, *, backend, backend_scale, units, loop_phase=..., plan_step=None)` | create a new ledger; rejects if one already exists; recomputes predicate; atomic write. `plan_step` defaults to `null` (no plan step yet) |
...
| `atomic_iterate_step(repo, run, gate_unit_id, producer, new_depends_on)` | **(v0.3.0, U2)** composite — wraps `iteration_attempts++` + emit + reset in ONE locked body (all-or-nothing). Engine-only caller (U4's `advance_iteration_loop`) |
...
| `exit_predicate_result.all_steps_terminal` | `all_steps_terminal` | the cached predicate | <!--legacy--> |
...
mapped record can never carry both `units` and `steps`. Genuinely-unknown keys pass
